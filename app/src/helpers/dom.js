...
const getColours = css => {
  let bg = css.match(/hljs\s*{.*?background:\s*(#[a-z0-9]{3,6})/i)[1]
  let fg = css.match(/hljs\s*{.*?color:\s*(#[a-z0-9]{3,6})/i)[1]
  let ln = css.match(/hljs-attribute[^{]*{.*?color:\s*(#[a-z0-9]{3,6})/i)[1]
