...
<div class="callout callout-alert"><p><span class="screen-reader-text">Alert:</span>
  ...
  "code": "rest_no_route",
    ...
    "data": {
      "status": 404
		...
		<p>The message &amp; code should tell you what the issue is and the
			data object contains the status code which corresponds to the
			HTTP status code.</p>
		<p>Another common error is failing the authentication checks for
			the action you are performing. You may see either of the following:</p>
		<pre><code>{
...
foreach ( get_post_types( array( 'show_in_rest' => true ), 'objects' ) as $post_type ) {
	...
	add_route_description( "/wp/v2/{$post_type->rest_base}/(?P<id>[\d]+)",
		sprintf(
...
foreach ( get_taxonomies( array( 'show_in_rest' => true ), 'objects' ) as $taxonomy ) {
	add_route_description( "/wp/v2/{$taxonomy->rest_base}",
		sprintf(
			__( 'List or create %s.', 'restsplain' ),
			$taxonomy->labels->name
		)
	);
	add_route_description( "/wp/v2/{$taxonomy->rest_base}/(?P<id>[\d]+)",
		sprintf(
			__( 'Fetch or edit a single %s.', 'restsplain' ),
...
// Users
add_route_description( '/wp/v2/users', __( 'Fetch or create Users.', 'restsplain' ) );
add_route_description( '/wp/v2/users/(?P<id>[\d]+)', __( 'Fetch or edit a single User.', 'restsplain' ) );
add_route_description( '/wp/v2/users/me', __( 'Fetch or edit the currently logged in User.', 'restsplain' ) );

// Types
add_route_description( '/wp/v2/types', __( 'Fetch information about all post types.', 'restsplain' ) );
add_route_description( '/wp/v2/types/(?P<type>[\w-]+)', __( 'Fetch information about a single post type.', 'restsplain' ) );

// Taxonomies
add_route_description( '/wp/v2/taxonomies', __( 'Fetch information about all taxonomies.', 'restsplain' ) );
add_route_description( '/wp/v2/taxonomies/(?P<taxonomy>[\w-]+)', __( 'Fetch information about a single taxonomy.', 'restsplain' ) );

// Statuses
add_route_description( '/wp/v2/statuses', __( 'Fetch information about available post statuses.', 'restsplain' ) );
add_route_description( '/wp/v2/statuses/(?P<status>[\w-]+)', __( 'Fetch information about a single post status.', 'restsplain' ) );

// Comments
add_route_description( '/wp/v2/comments', __( 'Fetch or create Comments.', 'restsplain' ) );
add_route_description( '/wp/v2/comments/(?P<id>[\d]+)', __( 'Fetch or edit a single Comment.', 'restsplain' ) );
